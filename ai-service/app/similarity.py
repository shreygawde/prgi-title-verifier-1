...
def _weighted_word_score(
    ...
    for left_index in sorted(
        range(len(left)),
        key=lambda index: _term_weight(left[index]),
        reverse=True,
    ):
        best_index = None
        best_score = 0.0
        ...
        weight = min(
            _term_weight(left[left_index]),
            _term_weight(right[best_index]),
        )

        evidence += weight * (best_score / 100.0)

    # Every word position contributes to the denominator.
    # Generic words contribute less evidence, but unmatched words
    # still prevent one shared word from dominating the title score.
    denominator = max(len(left), len(right))

    if denominator == 0:
        return 0.0

    return 100.0 * evidence / denominator
...
def _word_phonetic_score(left: str, right: str) -> float:
    ...
    left_codes = {
        code for code in doublemetaphone(left) if code
    }
    right_codes = {
        code for code in doublemetaphone(right) if code
    }
...
def compare_titles(
    ...
    normalized_new = " ".join(new_tokens)
    normalized_existing = " ".join(existing_tokens)
    ...
    lexical_score = _weighted_word_score(
        new_tokens,
        existing_tokens,
        _fuzzy_word_score,
    )
    ...
    core_score = _weighted_word_score(
        core_new_tokens,
        core_existing_tokens,
        _fuzzy_word_score,
    )

    canonical_new_tokens = strip_affixes(
        canonicalize(new_tokens)
    )
    canonical_existing_tokens = strip_affixes(
        canonicalize(existing_tokens)
    )

    canonical_score = _weighted_word_score(
        canonical_new_tokens,
        canonical_existing_tokens,
        _fuzzy_word_score,
    )
    ...
    # A phonetic signal should only dominate the final title score when it
    # represents a genuinely strong correspondence.
    #
    # Single-word phonetic variations such as:
    #   color / colour
    #
    # remain valid strong matches.
    #
    # Multi-word titles must have a very strong phonetic score before
    # phonetics are allowed to dominate. This prevents cases such as:
    #   Financial Chronicle / Financial Express
    #
    # from becoming an 80%+ PHONETIC match merely because "Financial"
    # happens to match.
    phonetic_is_strong = (
        has_distinctive_phonetic_pair
        and (
            (
                len(core_new_tokens) == 1
                and len(core_existing_tokens) == 1
            )
            or phonetic_score >= 90
        )
    )

    # Do not allow a weak multi-word phonetic signal to become the final
    # similarity score.
    final_score = (
        100.0
    ...
    if phonetic_is_strong and phonetic_score >= 80:
        match_types.append(
            "PHONETIC"
        )
    ...
    if (
        canonical_score >= 80
        and canonical_score > max(
            lexical_score,
            core_score,
        )
    ):
        match_types.append(
            "CROSS_LANGUAGE_MEANING"
        )

    if (
        not match_types
        and final_score >= SIMILARITY_DISPLAY_THRESHOLD
    ):
        match_types.append(
            "FUZZY"
        )
